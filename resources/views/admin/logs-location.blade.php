    ...
    <body class="sb-nav-fixed p-2">
            ...
            <div id="layoutSidenav_content">
                <main class="shadow rounded p-3">
                    <div class="container-fluid row">
                        <div class="col-xl-12 ">
                            <h1 class="mt-4">Logs by Location</h1>
                            <ol class="breadcrumb mb-4">
                                <li class="breadcrumb-item active">Monitor User activity logs by location.</li>
                                    <!-- <button type="submit" class='btn btn-success ms-3'onclick="tableToExcel()" >Convert to CSV File</button> -->
                            </ol>
                        </div>
                        <div class="col-xl-12 row">
                            <div class="col-md-4">
                                <label for="sel_location"><strong>Select Location</strong></label>
                                <select class="form-select mb-3" name="sel_location" id="sel_location" required>
                            ...
                            <div class="col-md-4">
                            <label for="inp_date"><strong>Select Date</strong></label>
                                <input type="date" class="form-control" name="inp_date" id="inp_date" required>
                            </div>
                        </div>

                        <div class="col-xl-12">
                            <table id="logs_location" class="table table-hover" style="width:100%">
                                <thead>
                                    <tr>
        ...
        <script>
            $(document).ready(function () { 
                ...
                var logs = JSON.parse('{!! json_encode($logs) !!}');
                var logs_location = $('#logs_location').DataTable({
                    order: [[6, 'desc']],
                    dom: 'Bfrtip',
                    buttons: [
                        {
                ...
                $("#sel_location, #inp_date").on("change", function() {
                    var loc = $("#sel_location").val();
                    var date = $("#inp_date").val();

                    logs_location.clear().draw();
                    logs.forEach(function (item) {
                        if(loc == item.location && date == item.time_in.substr(0,10)){
                            logs_location.row.add($("<tr><small><td>" + item.type + "</td></small><small><td>" + item.card_id + "</td></small><small><td>"+ item.identity_number +"</td></small><small><td>"+ item.name +"</td></small><small><td>"+ item.location +"</td></small><small><td>"+ item.time_in +"</td></small><small><td>"+ item.time_out + "</td></small></tr>")).draw(false);
                        }
                    });
                });
                $("#sel_location, #inp_date").trigger("change");
            });
        </script>
<script>
</script>
